...
//> STATIC CONSTANTS
	...
	// FIXME encode these unicode strings in \\uBLAHBLAH notation so that the unit tests actually pass on non-UTF-8 systems.  This *shouldn't* be necessary, but clearly is. 
	public static final IncomingTextSmsTestData[] UCS2_MESSAGES = {
		/*
		 * The following data is temporarily removed pending ticket #32 on trac:
		 * When compiling, Maven seems to ignore the character encoding which it is told to use. For this reason,
		 * tests based on org.smslib.IncomingTextSmsTestData? will fail if your default encoding is not UTF-8.
		 * The data in org.smslib.IncomingTextSmsTestData?.MULTIPART_UCS2_MESSAGES but with any unsafe characters
